          ...
          {user && (
            <div className="flex items-center ml-2 pl-4 border-l border-slate-200">
              <div className="mr-3 hidden sm:block text-right">
                ...
                <p className="text-[10px] text-slate-400 mt-1 uppercase font-black tracking-tighter">{user.email?.split('@')[0]}</p>
              </div>
              <div className="group relative">
                <button className="w-9 h-9 bg-eneo-violet rounded-full flex items-center justify-center text-white font-bold text-sm drive-shadow-sm hover:ring-4 hover:ring-violet-50 transition-all">
                  {user.firstName?.[0]}{user.lastName?.[0]}
                </button>
                <div className="absolute right-0 mt-2 w-48 bg-white border border-slate-100 rounded-2xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-[60] py-2">
                   <div className="px-4 py-2 border-b border-slate-50 mb-1">
                      <p className="text-xs font-bold text-slate-800">{user.email}</p>
                   </div>
                   <button 
                    onClick={logout}
                    className="w-full flex items-center space-x-3 px-4 py-2 text-rose-600 hover:bg-rose-50 text-sm font-bold transition-colors"
                   >
